...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    flag = False #signal the occurence of 1
    num_of_turns, result = 0, 0
    while num_of_turns < num_rolls:
        cur = dice()
        if(cur == 1):
            flag = True
        result += cur
...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    ones_digit, tens_digit = player_score % 10, (opponent_score // 10) % 10
    diff = abs(ones_digit - tens_digit)
    return max(1, 3 * diff)
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM 7
    org_roll = strategy(0,0)
    for player_score in range(goal):
        for opponent_score in range(goal):
            if strategy(player_score, opponent_score) != org_roll:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    for num in range(1,11):
        this_score = make_averaged(roll_dice,times_called)(num, dice)
