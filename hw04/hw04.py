...
def balanced(m):
    ...
    if is_planet(m):
        ...
        end_left, len_left = end(left(m)), length(left(m))
        end_right, len_right = end(right(m)), length(right(m))
        return len_left * total_mass(end_left) == len_right * total_mass(end_right) and balanced(end_left) and balanced(end_right)
...
def max_path_sum(t):
    ...
    max_sum = label(t)
    for branch in branches(t):
        max_sum = max(max_sum, label(t) + max_path_sum(branch))
    return max_sum
